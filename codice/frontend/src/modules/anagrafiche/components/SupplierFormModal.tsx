...
export function SupplierFormModal({ open, onClose, onSave, initialData, mode, nextId }: SupplierFormModalProps) {
  const [formData, setFormData] = useState<SupplierFormData>(
    initialData || {
      ragioneSociale: '',
      codice: `FOR-${String(nextId || 1).padStart(3, '0')}`,
      pIva: '',
      citta: '',
      email: '',
      telefono: '',
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              />
              {mode === 'create' && (
                <p className="text-xs text-[#6B7280] mt-1">Generato automaticamente</p>
              )}
            </div>
