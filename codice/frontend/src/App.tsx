...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    authApi.me()
      .then((u) => {
        const currentToken = localStorage.getItem('lc_token');
        if (currentToken) setAuth(currentToken, u);
      })
      .catch(() => {
      });
  }, []);
  ...
  const uiUser = {
    ...
    email:    utente.email,
    password: '',
    ruolo:    ruoloNome as import('./components/LogiChain_Auth').Role,
    avatar:   initials,
                ...
                {dashboardTab === 'alert' && (
                  <div className="space-y-5">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <select
                        value={filterTipo}
                        onChange={(e) => setFilterTipo(e.target.value as NotifType | 'tutti')}
                        className="px-3 py-1.5 border border-[#E5EAF2] rounded-lg text-sm text-[#374151] bg-white focus:outline-none focus:ring-2 focus:ring-[#17E88F]"
                      >
                      ...
                      <select
                        value={filterLetta}
                        onChange={(e) => setFilterLetta(e.target.value as 'tutti' | 'lette' | 'non_lette')}
                        className="px-3 py-1.5 border border-[#E5EAF2] rounded-lg text-sm text-[#374151] bg-white focus:outline-none focus:ring-2 focus:ring-[#17E88F]"
                      >
                      ...
                      <button
                        onClick={() => { setFilterTipo('tutti'); setFilterLetta('tutti'); }}
                        className="px-3 py-1.5 border border-[#E5EAF2] rounded-lg text-sm text-[#6B7280] bg-white hover:bg-[#F7F9FC] flex items-center gap-1.5 transition-colors"
                      >
                        ...
                        return (
                          <div
                            ...
                            className={`flex items-start gap-4 p-4 rounded-xl border transition-all ${
                              n.letta ? 'bg-white border-[#E5EAF2] opacity-60' : 'bg-[#FAFFFE] border-[#D1FAE5] shadow-sm'
                            }`}
                          >
                            ...
                            {!n.letta && (
                              <button
                                ...
                                className="flex-shrink-0 px-2.5 py-1 text-xs text-[#6B7280] border border-[#E5EAF2] rounded-lg hover:bg-[#F7F9FC] transition-colors"
                              >
...
export default function App() {
  const { token, utente, logout } = useAuthStore();
