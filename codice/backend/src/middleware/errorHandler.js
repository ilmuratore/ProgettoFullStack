const errorMessages = {
  ...
  CORRIERE_CON_SPEDIZIONI:
    "Impossibile eliminare: il corriere ha spedizioni associate",
  CATEGORIA_CON_PRODOTTI:
    "Impossibile eliminare: la categoria ha prodotti associati",
  CATEGORIA_CON_SOTTOCATEGORIE:
    "Impossibile eliminare: la categoria ha sottocategorie",
  ...
  INVALID_FILE_TYPE: "Tipo file non consentito",
  UPLOAD_ERROR: "Errore durante il caricamento file",
};
...
const errorHandler = (err, _req, res, _next) => {
  ...
  if (process.env.NODE_ENV !== "production") {
    console.error(
      `[${new Date().toISOString()}] ${statusCode} - ${code}: ${message}`,
    );
